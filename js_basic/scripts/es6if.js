...
plusBtn.addEventListener('click',()=>{
    ...
    //참이면 '최대구매수량입니다'
    //거짓이면 '수량증가'
    result = productDB[0].stock <= numInput.value ? alert('최대구매수량입니다') : ++num;
    numOperatorFunc(0);
})
//감소 버튼 클릭 시 현재 수량이 1보다 작으면 '최소구매수량입니다' 아니면 수량감소
minusBtn.addEventListener('click',()=>{
    result = num <= 1 ? alert('최소구매수량입니다'): --num;
    numOperatorFunc(0);
})
//증가, 감소 버튼 시 반복 실행하는 함수(0.1로 매번 다른 인덱스만 매개변수처리)
function numOperatorFunc(index){
    numInput.value = num;
    priceP.textContent = (productDB[index].price * num).toLocaleString('ko-kr')+`원`;
    console.log(result);
}