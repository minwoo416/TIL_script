...
const buyBtn = document.querySelector('#buyBtn'); //변수 생성 //해석은 오른쪽에서 왼쪽으로 : buyBtn을 문서에서 찾아서 앞쪽의 buyBtn에 대입해라
console.log(buyBtn); //변수 정상확인
...
//아이디중복확인 클릭 시 '아이다가 중복되지 않습니다. 사용하셔도 좋습니다' 출력 알고리즘
...
//pw 입력창 객체 준비 후 키보드를 아무키나 입력했을 떼 '비밀번호는 특수문자 포함 12글자 이상 입력하세요' 출력 알고리즘
...
//커서를 빼면 배경색 흰색
const itemNum = document.querySelector('input[name=item_num]');
...
//addEventListener이벤트 작성 시 'on' 없이 이벤트만 작성한다. //on은 태그에 작성시에만 작성
