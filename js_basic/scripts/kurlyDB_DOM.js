...
function loadMoreContent(){
    ...
    for(let i=startIndex; i<endIndex; i++){
        ...
        newLi.innerHTML += `<p class="price">${Math.round((kurlyDB[i].price * (1 - kurlyDB[i].sale / 100))).toLocaleString('ko-kr')}`
        newLi.innerHTML += `<p class="sale">${kurlyDB[i].sale}%</p>`
        newLi.innerHTML += `<pclass="review">${kurlyDB[i].review}+</p>`
...
window.addEventListener('scroll',()=>{
    ...
    //현재 스크롤 위치가 전체 스크롤 위치대배 80% 뷰포트 위치에 왔는지 + DB로드안된 상태인지
