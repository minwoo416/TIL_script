...
section.appendChild(ulClone); //section의 마지막자식위치에 복제붙여넣기(Ctrl+v)
...
let cssQ = confirm('background 통합속성을 사용할 때 size는 꼭 무조건 별도로 작성해야 한다. 맞으면 확인, 틀리면 취소를 누르세요');
while(cssQ != false){
    cssQ = confirm('background 통합속성을 사용할 때 size는 꼭 무조건 별도로 작성해야 한다. 맞으면 확인, 틀리면 취소를 누르세요');
}
alert('오....잘하시네요');

let jsQ = confirm('화살표함수는 ()=>{} 이다. 맞으면 확인, 틀리면 취소를 누르세요');
while(jsQ != true){
    jsQ = confirm('화살표함수는 ()=>{} 이다. 맞으면 확인, 틀리면 취소를 누르세요');
}
alert('오....마스터..?'); */
...
for(let i=1; i<3; i++){//첫번째 for 문
    for(let j=0;j<2;j++){//두번째 for 문
