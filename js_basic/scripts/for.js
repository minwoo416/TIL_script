...
//1번 메뉴 아메리카노
//2번 메뉴 카페라떼 ...반복
for(let i=0;i<coffee.length;i++){//커피의 메뉴가 늘어날 가능성이 있으므로 length를 사용한다.
    console.log((i+1)+'번 메뉴 '+coffee[i]);
}
//역순 카페 메뉴 출력
for(let i=coffee.length-1;i>=0;i--){//length는 1부터 세기때문에 -1을 해준다.
    console.log((i+1)+'번 메뉴 '+coffee[i])
}
//구구단 2단 ~ 9단 출력
//2단 2x1=2 형태로 출력하기
//2x1=(2*1)
//2x2=(2*2)
//2x3=(2*3) //고정부분 제외하고 변하는 부분을 for의 초기변수
for(let i=1;i<10;i++){
...
for(let i=0; i<navA.length;i++){
    ...
    if(i%2 == 1){
        ...
        //navA[i].style.background = '#ff0';
    }else {
        navA[i].classList.add('even');
        //navA[i].style.color = '#00f' //파랑
        //navA[i].style.background = '#eee';
    }
}
