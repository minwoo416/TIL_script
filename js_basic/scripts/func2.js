...
function scopetest(){ //함수 호출 전 대기상태의 함수 생성구조
    f = 25; //지역
    g = 35; //지역
    console.log(d+f); //지역 o 5+25
    console.log(e+g); //지역 o 15+35
}
console.log(g+d); //오류
console.log(d+e); //5+15
console.log('----------------------------');
...
function coffeeFunc1 (data1=1, data2=0.5){ //기본값설정
...
function coffeeOrder1 (coffee1, coffee2=1){
    /* let menu = coffee1
    let cup = coffee2
    console.log(`${coffee1} ${coffee2}잔 주문완료되었습니다.`) */
    const msg =`잔 주문완료되었습니다.`
    console.log(coffee1, coffee2, msg);
}
coffeeOrder1('카페라떼'); //기본값이라 두번째 매개변수 생략
coffeeOrder1('아메리카노',2); //기본값이 아니라 매개변수 모두 작성
coffeeOrder1('녹차라떼');
coffeeOrder1('아메리카노',4);
console.log('-------------------------커피주문 알고리즘 활용');
function coffeeOrder2 (temperature, menu, cup=1){
    const msg = '잔 주문완료되었습니다.';
    const type = ['따뜻한','차가운'];
    console.log(type[temperature], menu, cup, msg);
}
coffeeOrder2(0,'카페라떼',2);
coffeeOrder2(1,'아메리카노');
coffeeOrder2(0,'녹차라떼',2);
coffeeOrder2(1,'아메리카노');
...
function discount(price){
    discountResult = `5% 할인이 적용된 후 가격은 ${price*0.95} 원입니다. `;
    discountResult += `10% 할인이 적용된 후 가격은 ${price*0.9} 원입니다. `;
    discountResult += `20% 할인이 적용된 후 가격은 ${price*0.8} 원입니다. `;
    discountResult += `30% 할인이 적용된 후 가격은 ${price*0.7} 원입니다. `;
    discountResult += `40% 할인이 적용된 후 가격은 ${price*0.6} 원입니다. `;
    discountResult += `50% 할인이 적용된 후 가격은 ${price*0.5} 원입니다. `;
...
//함수 자체의 이름이 아닌 변수 이름에 의존해서 할당되기때문에 익명함수라고 부른다.
...
//대입연산자(=)와 화살표함수(=>)를 붙여서 사용할 수 없기때문에 매개변수와 함수선언표시 순서를 바꿔서 =()=> 로 사용한다.
//일반함수 경우
function bab(num){
    return '밥'+num+'공기 주세요';
}
console.log(bab(1))
//화살표함수(익명함수) 경우
const bab2 = (num) => {return '밥'+num+'공기주세요'};
console.log(bab2(2));
