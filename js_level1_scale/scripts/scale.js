...
//객체.속성
//사용자정의함수 없이 속성 및 내장함수를 작성하면 사용자 동작과 관계없이 즉시 실행된다.
console.log(`사용자가 입력한 키 : ${userHeight.value}`);
console.log(`사용자가 입력한 몸무게 : ${userWeight.value}`);
//사용자가 키를 입력한 후 프로그램이 그 값을 인식하게 만드는 법
//값을 실시간으로 입력하는 동안은 프로그램 과부하 및 보안 문제로 함수를 작동하게 해선 안되며 결정버튼을 따로 만들어두고 버튼에 특정동작을 해야만 함수가 실행되게 만들어야 한다.
...
function weightCal(){
    ...
    console.log(`당신의 적정체중은 ${normalWeight}kg 이며 ${userWeight.value-normalWeight}kg 초과되셨습니다.`)
    //input 요소일 경우 값 읽기/수정/삭제는 value 속성 사용
    //input이 아닌 다른 요소일경우 값 읽기/수정/삭제 textContent 또는 innerHTML 속성 사용
    result.innerHTML = `당신의 적정체중은 ${normalWeight}kg 이며<br> ${userWeight.value-normalWeight}kg <em>초과</em>되셨습니다.`
}
