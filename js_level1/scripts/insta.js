...
likeImg.addEventListener('click',function(e){
    //console.log(e);
    e.preventDefault();//태그의 동적기능 취소(a태그의 새로고침)
    likeOn();
});



// 6.댓글 아이콘 터치 시 댓글 입력창 실행(o), 실행화면에서 바깥쪽 영역 터치 댓글입력창 닫힘
// 7.공유 아이콘 터치 시 공유가능 계정들 표시된 창 실행(o)-제작필요, 바깥쪽 영역 터치 시 닫힘 (o)
// 8.즐겨찾기 아이콘 터치 시 즐겨찾기 저장(x-DB필요)이 됨과 동시에 컬렉션 선택화면 출력(o), 바깥쪽 영역 터치 시 닫힘 
...
// 11.날짜 터치 기능 없음(x js)
// 추가 페이지 목록) 프로필페이지(insta_profile.html), 저장~신고 등 기능, 댓글 입력창, 공유가능 계정들 표시, 컬렉션 선택 화면, 태그 사용 게시글(insta_tag.html)